...
df = spark.read.table("uber_catalog.bronze.bulk_rides")
df = df.withColumn("booking_timestamp", col("booking_timestamp").cast("timestamp"))
df_bulk = df.select(*select_fields_list)


df_bulk.show(10, False)
...
df = spark.read.table("uber_catalog.bronze.rides_raw")
df_parsed = df.withColumn("parsed_rides", from_json(col("rides"), rides_schema))\
            .select("parsed_rides.*")
df_rides = df_parsed.select(*select_fields_list)
df_rides.show(10, False)

# COMMAND ----------

df_union = df_rides.union(df_bulk)
df_union.show(10, False)